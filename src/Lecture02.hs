...
filter f Nil = Nil
filter f (a :. b) = foldr (\c d -> if f c then c :. d else d) Nil (a :. b)
...
toListH :: List a -> [a]
toListH Nil = []
toListH (a :. b) = a : toListH b

-- И обратно
fromListH :: [a] -> List a
fromListH [] = Nil
fromListH (a : b) = a :. fromListH b
...
intercalate :: [a] -> [[a]] -> [a]
intercalate a [] = []
intercalate a (b : c) = P.foldl(\d f -> d ++ a ++ f) b c
